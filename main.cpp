        ...
        if (event == "telemetry") {
          ...
          //
		 	...
		 	for (unsigned int i=0; i < ptsx.size(); i++) 
			{
				...
				std::cout << "x_carcoords[i] = " << ptsx_carcoords[i] << "\t" << 
								"y_carcoords[i] = " << ptsy_carcoords[i] << "\n"; 
     		}

			//=======================================================
      		// fit a polynomial for our waypoints
     		auto coeffs = polyfit(ptsx_carcoords, ptsy_carcoords, 2);
			//=======================================================
			...
			// v_[t+1] = v[t] + a[t] * dt
			//-----------------------------------------------
			double predicted_x    = px + v * mpc.dt;		
			double predicted_y    = py;
			double predicted_psi  = v * steer/ mpc.Lf * mpc.dt;
			double predicted_v    = v + mpc.prev_a * mpc.dt;
			
			double predicted_cte  = polyeval(coeffs, predicted_x);
			double predicted_epsi = -atan(coeffs[1]);
